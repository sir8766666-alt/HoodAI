...
# -----------------------------------------------------------------------------
# Admin-only: reset a user's password directly, no email
# -----------------------------------------------------------------------------

@app.post("/admin/reset-password")
def admin_reset_password(payload: AdminResetPasswordIn, x_admin_secret: Optional[str] = Header(None)):
    """
    Sets a Supabase Auth password directly via the Admin API. No email,
    no reset link — the new password is active immediately.

    Requires X-Admin-Secret. Never callable from any frontend code.
    The service-role client used everywhere else in this file already
    has admin rights, so no extra credential is needed beyond that.
    """
    require_admin(x_admin_secret)

    lookup = supabase.table("users").select("email").eq("email", payload.email.strip().lower()).limit(1).execute()
    if not lookup.data:
        raise HTTPException(status_code=404, detail="No HoodAI user found with that email")

    # Find the matching Supabase Auth user by email.
    try:
        auth_users = supabase.auth.admin.list_users()
    except Exception:
        raise HTTPException(status_code=500, detail="Could not reach Supabase Auth")

    target_auth_user = None
    for u in auth_users:
        if getattr(u, "email", "").strip().lower() == payload.email.strip().lower():
            target_auth_user = u
            break

    if not target_auth_user:
        raise HTTPException(status_code=404, detail="No Supabase Auth account found with that email")
    ...
    return {"success": True, "message": "Password updated. No email was sent."}
